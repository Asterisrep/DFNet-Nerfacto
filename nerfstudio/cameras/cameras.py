...
class Cameras(TensorDataclass):
    ...
    def _generate_rays_from_coords(
        ...
        # # Convert true_indices_flat to a PyTorch tensor
        # true_indices_tensor = torch.tensor(true_indices_flat)

        # # Move true_indices_tensor to the same device as self.distortion_params
        # true_indices_tensor = true_indices_tensor.to(self.distortion_params.device)

        # Now use true_indices_tensor for indexing
        
        
        distortion_params = self.distortion_params.to('cuda')
        if not disable_distortion:
            ...
            if distortion_params is not None:
                ...
                mask = mask.to(distortion_params.device)
