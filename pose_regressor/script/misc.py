...
def render_nerfw_imgs(args, dl, hwf, device, nerfacto_model, nerfacto_params):
    ...
    for batch_idx, (target, pose, img_idx) in enumerate(dl):
        ...
        rgb = vizualize(pose_nerf, nerfacto_model, nerfacto_params)
...
def get_render_error_in_q(args, model, sample_size, device, rgbs, poses, batch_size, basedir, expname):
    ...
    ang_error_list = np.array(ang_error_list)
    vis_info_ret = {"pose": predict_pose_list, "pose_gt": gt_pose_list, "theta": ang_error_list}
    #print(predict_pose_list, gt_pose_list, ang_error_list)
...
def create_perturbed_testset(poses, x, angle):
    """
    Inputs:
        poses: (N_views, 3, 4) containing all ground truth poses for every image in train dataset.
        x: translational perturb range
        angle: rotation angle perturb range in degrees
    Outputs:
        new_c2w: (N_views, 3, 4) new poses with perturbations applied every 40 poses.
    """
    N_views = 1  # number of views
    new_c2w = np.zeros((N_views, 3, 4))
    ...
    for i in range(N_views):
        # Copy current pose
        new_c2w[i] = deepcopy(poses[i])
        
        if i % 40 == 0:  # Apply perturbation every 40th pose
            loc = deepcopy(new_c2w[i, :, 3])

            # Perturb rotation pose
            rot_rand = np.random.uniform(-angle, angle, 3)  # in degrees
            theta, phi, psi = rot_rand
            theta = phi = 0  # Assuming you still want to zero these out as in your original function

            new_c2w[i] = perturb_rotation(new_c2w[i], theta, phi, psi)

            # Perturb translational pose
            trans_rand = np.random.uniform(-x, x, 2)  # Random numbers for x and y axes
            trans_rand_3 = np.append(trans_rand, 0)  # Append 0 for the z axis

            new_c2w[i, :, 3] = loc + trans_rand_3  # Apply translational perturbation
