...
def config_parser():
    ...
    parser.add_argument("--N_rand", type=int, default=2000, help='batch size (number of random rays per gradient step)')
    ...
    parser.add_argument("--tinyscale", type=float, default=2., help='determine the scale of downsizing nerf rendring, must FIX later')
    ...
    parser.add_argument("--DFNet", action='store_true', default=True, help='use DFNet')
    ...
    parser.add_argument("--rvs_refresh_rate", type=int, default=15, help='re-synthesis new views per X epochs')
    parser.add_argument("--rvs_trans", type=float, default=0.1, help='jitter range for rvs on translation')
    parser.add_argument("--rvs_rotation", type=float, default=20, help='jitter range for rvs on rotation, this is in log_10 uniform range, log(15) = 1.2')
    ...
    #parser.add_argument("--feature_metric_level", type=int, default=[0], help='batch_size for validation, higher number leads to faster speed')
    ...
    #parser.add_argument("--combine_loss", action='store_true', default=True, help='combined l2 pose loss + rgb mse loss')
    ...
    parser.add_argument("--tripletloss", action='store_true', default=False, help='use triplet loss at training featurenet, this is to prevent catastophic failing')
    ...
    parser.add_argument("--output_path",  type=str, default="iw_output/init_pose/simple.txt", help='path to the file saving poses')
